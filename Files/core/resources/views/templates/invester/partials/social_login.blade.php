...
<div class="my-2">
    @if (@gs('socialite_credentials')->google->status == Status::ENABLE)
        <a href="{{ route('user.social.login', 'google') }}" class="btn btn--outline cmn--btn mt-3 w-100"><img src="{{ asset('assets/global/images/google.svg') }}" alt="@lang('image')" class="others-login-image"> @lang("$text with Google")</a>
    @endif

    @if (@gs('socialite_credentials')->facebook->status == Status::ENABLE)
        <a href="{{ route('user.social.login', 'facebook') }}" class="btn btn--outline cmn--btn mt-3 w-100"><img src="{{ asset('assets/global/images/facebook.svg') }}" alt="@lang('image')" class="others-login-image"> @lang("$text with Facebook")</a>
    @endif

    @if (@gs('socialite_credentials')->linkedin->status == Status::ENABLE)
        <a href="{{ route('user.social.login', 'linkedin') }}" class="btn btn--outline cmn--btn mt-3 w-100"><img src="{{ asset('assets/global/images/linkedin.svg') }}" alt="@lang('image')" class="others-login-image"> @lang("$text with Linkedin")</a>
    @endif
